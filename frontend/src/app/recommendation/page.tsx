...
function LoadingState({ reviewCount, restaurantName, analysisSteps, analysisStep }: LoadingStateProps) {
    return (
        <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center space-y-8">
            ...
            <div className="w-full max-w-md bg-secondary/50 rounded-xl p-4 space-y-3">
            ...
            <div className="absolute inset-0 w-full h-full overflow-y-auto">
                ...
                <div className="bg-background/95 backdrop-blur-sm sticky top-14 z-10 px-6 py-4 shadow-sm border-b">
                    <div className="flex justify-between items-end mb-2">
                        <div>
                            <p className="text-xs text-muted-foreground mb-1 uppercase tracking-wider">菜單總價</p>
                            <h1 className="text-3xl font-bold text-foreground font-mono transition-colors duration-300">NT$ {totalPrice.toLocaleString()}</h1>
                        </div>
                        <div className="text-right">
                            <p className="text-xs text-muted-foreground mb-1">人均約</p>
                            <p className="text-xl font-bold text-orange-600 font-mono">NT$ {perPerson.toLocaleString()}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-4 space-y-4">
                    {dishSlots.map((slot, index) => {
                        ...
                        return (
                            ...
                            >
                                <Card className={`p-4 transition-all duration-300 rounded-xl bg-white shadow-sm ${status === 'selected' ? 'opacity-50' : ''}`}>
                                    <div className="flex gap-4">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-start mb-2">
                                                <h3 className="font-bold text-lg text-foreground leading-tight">{slot.display.dish_name}</h3>
                                                <span className="text-lg font-mono font-semibold text-foreground">NT$ {slot.display.price}</span>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-2 mt-4">
                                                ...
                                                <Button
                                                    ...
                                                    disabled={isSwapping}
                                                >
                                                    <RotateCw className={`w-4 h-4 mr-1.5 ${isSwapping ? 'animate-spin' : ''}`} />
                                                    換一道
                                                </Button>
                                            </div>
                                        </div>
